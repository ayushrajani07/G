...
# -*- coding: utf-8 -*-
"""
Ultimate Stable Launcher - G6.1 Platform (ENHANCED DATA STREAM VERSION)
Advanced launcher with enhanced rolling live data stream, improved columns, and alternating colors

ENHANCEMENTS IMPLEMENTED:
- Added AVG column (day average for corresponding index)
- Removed Premium ₹L, Volume, and Thru/s columns
- Added Symmetric and Asymmetric offset counts as columns
- Added Description column with failure reasons when status is ❌
- Increased last updates from 15 to 25
- Alternating colors for every cycle logs
- Enhanced stream summary without premium
"""
...
class EnhancedRollingDataPoint:
    ...
    index: str
    legs: int
    avg_legs: float  # NEW: Day average for corresponding index
    success_rate: float
    symmetric_offsets: int  # NEW: Symmetric offset count
    asymmetric_offsets: int  # NEW: Asymmetric offset count
    status: str
    description: str  # NEW: Reason when status is ❌
    cycle_color: str  # NEW: Alternating color for cycle
...
class EnhancedRollingDataStream:
    ...
    def __init__(self, max_entries: int = 100):  # Increased capacity
    ...
    def get_recent_data(self, count: int = 25) -> List[EnhancedRollingDataPoint]:  # Increased to 25
...
class UltimateEnhancedLauncher:
    """Ultimate enhanced launcher with improved data stream columns and alternating colors."""
    ...
    def __init__(self):
        ...
        self.data_stream = EnhancedRollingDataStream()  # Enhanced data stream
...
[dim]Note: Option 1 requires Flask and will open your web browser[/dim]
    ...
    def setup_layout(self):
        ...
        try:
            ...
            self.layout["data_collection"].update(Panel("", title="📈 Enhanced Rolling Live Data Stream", border_style="blue"))
    ...
    def _create_header(self) -> Panel:
        ...
        try:
            ...
            header_text = Text()
            header_text.append("🚀 G6.1 Enhanced Platform", style="bold blue")
            header_text.append(" | ")
            ...
            return Panel(Align.center("🚀 G6.1 Enhanced Platform"), box=box.HEAVY)
    
    def _create_footer(self) -> Panel:
        """Create footer panel."""
        try:
            footer_text = "[dim]Press Ctrl+C to exit | Enhanced rolling data stream active | Updates every 15 seconds[/dim]"
    ...
    def _update_enhanced_rolling_data_panel(self):
        ...
        try:
            if not self.data_collection_active:
                init_panel = Panel(
                    Align.center("📊 Initializing enhanced rolling data stream...\n\n[dim]Preparing enhanced data collection with alternating colors[/dim]"),
            ...
            data_table.add_column("AVG", style="blue", width=6)  # NEW: Day average
            data_table.add_column("Success", style="yellow", width=8)
            data_table.add_column("Sym Off", style="magenta", width=7)  # NEW: Symmetric offsets
            data_table.add_column("Asym Off", style="cyan", width=8)    # NEW: Asymmetric offsets
            data_table.add_column("Status", style="bold", width=6)
            data_table.add_column("Description", style="red", width=15)  # NEW: Failure description
            ...
            # Enhanced summary statistics (removed premium, clarified avg throughput)
            if recent_data:
                ...
                # Average throughput = average legs processed per time unit
                ...
                summary_text.append(f" | {avg_throughput:.1f} avg legs/update", style="bold white")  # Clarified avg throughput
    ...
    def run_monitoring_loop(self):
        """Run the enhanced monitoring loop with alternating colors and new columns."""
        if not RICH_AVAILABLE:
        ...
        try:
                ...
                while self.running:
                    try:
                        ...
                        if current_time - self.last_update >= 15.0:
                            ...
                            # Generate enhanced rolling data stream with alternating colors
                            self.data_stream.simulate_enhanced_data_stream()
                            
                            # Create new panels
                            new_header = self._create_header()
                            new_data_panel = self._update_enhanced_rolling_data_panel()  # Enhanced data panel
            ...
            # Display persists on exit with enhanced session summary
            if RICH_AVAILABLE and CONSOLE:
                CONSOLE.print("\n" + "="*80)
                CONSOLE.print("🚀 [bold blue]G6.1 Enhanced Platform Shutdown Summary[/bold blue]")
                CONSOLE.print("="*80)
                
                stats = self.metrics_collector.comprehensive_stats
                uptime_hours = stats['uptime_seconds'] / 3600
                
                summary_table = Table(title="Enhanced Session Summary", box=box.ROUNDED)
                ...
                CONSOLE.print(f"\n✅ [green]Enhanced platform shutdown complete - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}[/green]")
                CONSOLE.print("📊 Enhanced rolling data stream with alternating colors preserved")
            else:
                print("\n✅ Enhanced platform shutdown complete")
    
    def _run_basic_monitoring(self):
        """Run monitoring in basic mode with enhanced rolling data."""
        print("\n" + "="*60)
        print("📊 G6.1 ENHANCED PLATFORM MONITORING (Basic Mode)")
        print("="*60)
        print("Enhanced rolling data stream with alternating colors active")
        ...
        try:
            ...
            while self.running:
                cycle += 1
                print(f"\n--- Enhanced Rolling Update {cycle} at {datetime.now().strftime('%H:%M:%S')} ---")
                ...
                if recent_data:
                    ...
                    for dp in recent_data:
                        ...
                        print(f"  {time_str} | {dp.index} | {dp.legs} legs (avg: {dp.avg_legs:.1f}) | {dp.success_rate:.1%} | Sym: {dp.symmetric_offsets} Asym: {dp.asymmetric_offsets} | {dp.status}{desc_str}")
            ...
            print("\n" + "="*60)
            print("🚀 G6.1 ENHANCED PLATFORM SHUTDOWN SUMMARY")
            print("="*60)
            stats = self.metrics_collector.comprehensive_stats
            uptime_hours = stats['uptime_seconds'] / 3600
            
            print(f"Session Duration: {uptime_hours:.2f} hours")
            print(f"Enhanced Rolling Data Points: {len(self.data_stream.data_points):,}")
            print(f"Color-Coded Warnings: {len(self.metrics_collector.warnings):,}")
            print(f"Total Collections: {stats['total_collections']:,}")
            print("✅ Enhanced platform shutdown complete - All enhanced data preserved")
    ...
    def launch(self):
        ...
        try:
            ...
            # Start platform
            if RICH_AVAILABLE:
                self.console.print("\n🚀 [bold green]Starting G6.1 Enhanced Platform...[/bold green]")
                self.console.print("📊 [cyan]Enhanced rolling data stream with AVG, offsets, descriptions, and alternating colors[/cyan]")
                self.console.print("💭 [dim]Press Ctrl+C to exit | Enhanced flicker-free display guaranteed[/dim]\n")
            else:
                print("\n🚀 Starting G6.1 Enhanced Platform...")
                print("📊 Enhanced rolling data stream with alternating colors active")
                print("💭 Press Ctrl+C to exit\n")
...
def main():
    """Main entry point."""
    try:
        launcher = UltimateEnhancedLauncher()
        success = launcher.launch()
        
        if success:
            print("\n👋 Thanks for using G6.1 Enhanced Platform!")
        else:
